...
{  
  ...
  if(command[0] == 0xFF && checksum_Test(command) == 0)
  {
    ...
    ** Joe custom commands
    *********************************************************************************/ 
    case 0x2F:     //Print Flowmeter total count
      serial_print_ulong(total_count_);
      Serial.flush();      
    break;
...
void serial_print_uint(unsigned int uint)
{
  Serial.write(uint >> 8);
  Serial.write(uint & 0xFF);
}

void serial_print_ulong(unsigned long ulong)
{
  Serial.write(ulong >> 24);
  Serial.write(ulong >> 16);
  Serial.write(ulong >> 8);
  Serial.write(ulong & 0xFF);
}
